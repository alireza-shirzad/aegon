...
//! stage of the pipeline: setup, commit, update_state, open, verify.
//!
...
/// divan shows readable bench names like `nv=16/k=2/zk=true/bool=false`.
#[derive(Clone, Copy)]
struct BenchCase {
    nv: usize,
    k: usize,
...
impl fmt::Debug for BenchCase {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "nv={}/k={}/zk={}/bool={}",
            self.nv, self.k, self.zk, self.boolean
        )
    }
}

const fn case(nv: usize, k: usize, zk: bool, boolean: bool) -> BenchCase {
    BenchCase { nv, k, zk, boolean }
}

/// Sweep points. Edit this list to add or trim bench configurations.
const CASES: &[BenchCase] = &[
    // case(16, 2, false, false),
    // case(16, 2, false, true),
    // case(16, 2, true, false),
    // case(16, 2, true, true),
    // case(17, 2, true, true),
    // case(18, 2, true, true),
    // case(19, 2, true, true),
...
fn prepare(c: BenchCase) -> Prepared {
    ...
    let poly = DenseOrSparseMLE::Dense(DenseMultilinearExtension::<Fr>::rand(c.nv, &mut rng));
    let point = sample_point(c.nv, c.boolean, &mut rng);
    (ck, vk, poly, point)
}

// ---------------------------------------------------------------------------
// setup (boolean is ignored)
// ---------------------------------------------------------------------------

#[divan::bench(args = CASES)]
fn setup(bencher: Bencher, c: &BenchCase) {
    let c = *c;
    bencher.bench_local(|| {
        let mut rng = test_rng();
        let srs =
            KZHK::<E>::gen_srs_for_testing(KZHKConfig::new(c.k, c.zk), &mut rng, c.nv).unwrap();
        black_box(srs);
    });
}
