...
use akd::aegon::{
    verify_sharded_lookup, AegonConfig, EcVrfHash, ShardTransport, ShardedAegon,
...
async fn grpc_sharded_publish_lookup_verify_roundtrip_ecvrf() {
    ...
    let commit = tokio::task::spawn_blocking(move || {
        let mut server = coord_clone.blocking_lock();
        server.publish(&updates_for_publish).expect("publish")
    })
    .await
    .expect("publish join");
    assert_eq!(commit.epoch, 1);
    ...
    for (label, value) in &updates {
        ...
        // Every probe along the trail must carry a 80-byte VRF proof.
        for (i, probe) in proof.probes.iter().enumerate() {
            assert_eq!(
                probe.vrf_proof.len(),
                80,
                "probe {i} of {label:?}: vrf_proof should be exactly 80 bytes (RFC 9381)"
            );
        }

        let ok = verify_sharded_lookup::<Bn254, Pcs, EcVrfHash>(
            &ctx, &commit, label, value, &proof,
        )
        .expect("verify ECVRF");
        assert!(
