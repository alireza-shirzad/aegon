...
use crate::aegon::{
    optimal_kzh_k, DbSource, EcVrfHash, ShardedAegon, ShardedAegonConfig,
    ShardedConsistencyProofTwoLayer, ShardedEpochCommitment, ShardedLabelHistory,
    ShardedLookupProofTwoLayer, ShardedValueHistory, ShardedVerifierContext, VrfProver,
};
...
fn default_aegon_config(db: DbSource) -> ShardedAegonConfig<DirectoryE, DirectoryPcs> {
...
pub struct Directory<TC, S: Database, V> {
    ...
    /// Which store the engine was built with. Kept so the history
    /// methods can tell "this label has no history" apart from "this
    /// directory has nowhere to read history from" — see
    /// [`Self::value_history`].
    db: DbSource,
    tc: PhantomData<TC>,
}
    ...
    V: VRFKeyStorage,
{
    ...
    pub async fn new(
        ...
        Self::new_with_db(storage, vrf, parallelism_config, DbSource::None).await
    }

    /// Like [`Self::new`], but attaches a key-value store to the
    /// engine.
    ///
    /// [`Self::new`] passes [`DbSource::None`], which is what the
    /// in-process tests want: no durability, and lookups served from
    /// the directory's own plaintext cache. That is also why
    /// [`Self::value_history`] and [`Self::label_history`] cannot
    /// work under it — history is read back out of the store, and
    /// there is no store.
    ///
    /// Pass [`DbSource::Rocks`] to get working history. Each
    /// in-process shard opens its own RocksDB under
    /// `<path>.shards/<i>`; the coordinator uses `<path>` itself.
    /// [`DbSource::Redis`] is rejected at setup for in-process
    /// shards, because the per-epoch opening keys carry no shard
    /// discriminator and shards sharing one keyspace would overwrite
    /// each other.
    ...
    /// **Not implemented in the Aegon backend — use
    /// [`Self::value_history`].** Calling this method panics.
    ///
    /// The capability exists; this signature cannot express it.
    /// [`HistoryProof`] is a Merkle object — per-version update
    /// proofs plus membership and non-membership proofs over marker
    /// versions — and Aegon produces no evidence of that kind. Its
    /// history is a window of polynomial openings anchored under the
    /// sharded Merkle root at each epoch a value changed, which is a
    /// different object rather than a differently-encoded one.
    ///
    /// Returning one dressed as the other would mean zero-filling
    /// every structured field and smuggling the real bundle through a
    /// byte vector, as [`Self::lookup`] does with
    /// `LookupProof::commitment_nonce`. That trade — compiling
    /// against unmodified AKD call sites, at the cost of proofs that
    /// fail verification in ways that look like proof failures rather
    /// than API mismatches — is not worth repeating for a method
    /// whose replacement is one rename away.
    ///
    /// See also [`Self::label_history`] for where and when a label was
    /// placed.
    #[cfg_attr(feature = "tracing_instrument", tracing::instrument(skip_all))]
    pub async fn key_history(
        ...
        unimplemented!(
            "AKD-on-Aegon: HistoryProof is a Merkle object (per-version update proofs, \
             membership/non-membership proofs over marker versions) and Aegon produces no \
             such evidence. The equivalent capability is Directory::value_history (what a \
             label's value was at each epoch) and Directory::label_history (where and when \
             the label was placed); verify them with aegon_facade::verify_value_history / \
             verify_label_history"
        )
    }

    /// A label's value history: one entry per epoch in which the
    /// label's value changed, most recent first and capped at
    /// [`crate::aegon::HISTORY_WINDOW`], plus a freshness attestation
    /// binding the newest entry to the live epoch.
    ///
    /// This is the Aegon analogue of AKD's [`Self::key_history`].
    /// It is *not* the same object: AKD proves a version chain with
    /// Merkle membership and non-membership proofs over marker
    /// versions, whereas each entry here is a pair of polynomial
    /// openings anchored under the sharded Merkle root at the epoch
    /// before and the epoch after the change. Verify with
    /// [`crate::aegon_facade::verify_value_history`].
    ///
    /// Requires a store: see [`Self::new_with_db`]. Under
    /// [`DbSource::None`] this returns an error rather than an empty
    /// history, so "nowhere to read from" cannot be mistaken for
    /// "this label never changed".
    pub async fn value_history(
        &self,
        akd_label: &AkdLabel,
    ) -> Result<ShardedValueHistory<DirectoryE, DirectoryPcs>, AkdError> {
        self.require_db("value_history")?;
        let aegon = self.aegon.lock().await;
        aegon.lookup_history(&akd_label.0).map_err(|e| {
            AkdError::Directory(DirectoryError::Publish(format!("aegon value_history: {e}")))
        })
    }

    /// A label's placement record: which shard and slot it was
    /// assigned, at which epoch, with the per-shard commitment and
    /// Merkle path from that epoch, plus a live freshness opening
    /// proving the placement still holds.
    ///
    /// Verify with [`crate::aegon_facade::verify_label_history`].
    /// Requires a store, on the same terms as [`Self::value_history`].
    pub async fn label_history(
        &self,
        akd_label: &AkdLabel,
    ) -> Result<ShardedLabelHistory<DirectoryE, DirectoryPcs>, AkdError> {
        self.require_db("label_history")?;
        let aegon = self.aegon.lock().await;
        aegon.lookup_label_history(&akd_label.0).map_err(|e| {
            AkdError::Directory(DirectoryError::Publish(format!("aegon label_history: {e}")))
        })
    }

    /// History is served out of the engine's store. Without one the
    /// underlying call returns an empty bundle, which is
    /// indistinguishable from a label that genuinely has no history —
    /// so refuse up front instead.
    fn require_db(&self, method: &str) -> Result<(), AkdError> {
        match self.db {
            DbSource::None => Err(AkdError::Directory(DirectoryError::Publish(format!(
                "Directory::{method} requires a key-value store; this Directory was built with \
                 DbSource::None. Use Directory::new_with_db(.., DbSource::Rocks(path))."
            )))),
            _ => Ok(()),
        }
    }
