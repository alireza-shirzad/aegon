...
// The Aegon-native verifier surface. Re-exported here because these
// are the replacements for `client::lookup_verify` / `auditor::*`,
// which `unimplemented!()` on this backend — a caller reaching for
// verification should find these at the crate root, where the legacy
// names used to be.
pub use aegon_facade::{
    verify_consistency, verify_invariance, verify_label_history, verify_lookup,
    verify_value_history,
};
