...
fn init_rejects_inconsistent_private_flag() {
    ...
    assert!(matches!(result, Err(aegon::AegonError::Config(_))));
}
...
fn verify_rejects_wrong_value() {
    ...
    assert!(matches!(result, Err(aegon::AegonError::Verification(_))));
}
...
fn lookup_unknown_label_errors() {
    ...
    assert!(matches!(result, Err(aegon::AegonError::UnknownLabel(_))));
}
...
fn end_to_end_two_batches_with_idle_epochs() {
    ...
    for (label, expected_value) in batch1.iter().chain(batch2.iter()) {
        ...
        assert!(
            matches!(bad, Err(aegon::AegonError::Verification(_))),
...
fn consistency_proof_unknown_epoch_errors() {
    ...
    assert!(matches!(result, Err(aegon::AegonError::InvalidEpoch(_))));
}
