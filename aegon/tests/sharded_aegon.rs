...
fn two_layer_publish_lookup_round_trip() {
    use aegon::verify_sharded_lookup_two_layer;
...
fn two_layer_publish_lookup_history_round_trip() {
    use aegon::{verify_lookup_history, verify_lookup_label_history, DbSource};
...
fn two_layer_consistency_proof_round_trip() {
    use aegon::verify_sharded_consistency_two_layer;
    let log_capacity = 6usize;
    let log_n_shards = 1usize;
    let mut server = fresh(log_capacity, log_n_shards);
...
fn private_mode_lookup_history_round_trip() {
    ...
    use aegon::{verify_lookup_history, DbSource};
    let tmp_root = std::env::temp_dir();
    let nonce: u64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_nanos() as u64)
...
fn builder_validation_rejects_bad_configs() {
    use aegon::{ShardTransport, SrsSource};

    fn must_err<T>(r: Result<T, aegon::AegonError>, needle: &str, label: &str) {
        match r {
            Err(e) => assert!(
    ...
    // More headroom gives each shard more slots.
    let cfg = ShardedAegonConfig::<Bn254, Pcs>::builder()
        .log_capacity(10)
        .log_over_provisioning_factor(3)
        .log_n_shards(2)
        .build()
        .expect("log factor 3 builds");
...
fn bench_setup_and_publish() {
    ...
    for (label, value) in &updates {
        let (_db_value, proof) = server.lookup_two_layer(label).expect("lookup");
        let ok = aegon::verify_sharded_lookup_two_layer::<Bn254, Pcs, Sha256Hash>(
            &ctx, &commit, label, value, &proof,
        )
        .expect("verify_sharded_lookup");
        assert!(ok);
