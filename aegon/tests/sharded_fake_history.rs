...
//!
//! Drives `ShardedAegon` directly. On every epoch we exercise:
//!
...
//!   * `verify_sharded_lookup_two_layer` — users pull their own slot, the
//!     verifier re-derives the cross-shard probe trail from the label.
//!   * `verify_sharded_consistency_two_layer` — users prove their slot has not
