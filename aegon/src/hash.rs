...
pub const VRF_PROOF_BYTES: usize = aegon_crypto::ecvrf::PROOF_LENGTH;
...
fn vrf_secret_key() -> &'static aegon_crypto::ecvrf::VRFPrivateKey {
    use aegon_crypto::ecvrf::VRFPrivateKey;
...
pub struct VrfProver {
    sk: aegon_crypto::ecvrf::VRFPrivateKey,
    pk: aegon_crypto::ecvrf::VRFPublicKey,
}

impl VrfProver {
    /// Build a prover from a 32-byte Ed25519 seed.
    pub fn from_seed(seed: &[u8; 32]) -> Self {
        let sk = aegon_crypto::ecvrf::VRFPrivateKey::try_from(seed.as_slice())
            .expect("32-byte seed is a valid Ed25519 secret");
        let pk = aegon_crypto::ecvrf::VRFPublicKey::from(&sk);
        Self { sk, pk }
    }
    ...
    pub fn public_key(&self) -> &aegon_crypto::ecvrf::VRFPublicKey {
        &self.pk
    }
...
impl VrfVerifier {
    ...
    pub fn from_public_key_bytes(pk_bytes: &[u8]) -> Result<Self, VrfVerifyError> {
        let pk = aegon_crypto::ecvrf::VRFPublicKey::try_from(pk_bytes)
            .map_err(|e| VrfVerifyError::Malformed(format!("{e:?}")))?;
        Ok(Self { pk })
    }

    /// Build a verifier directly from a parsed `VRFPublicKey`.
    pub fn new(pk: aegon_crypto::ecvrf::VRFPublicKey) -> Self {
        Self { pk }
    }
    ...
    fn verify_with_tag(
        ...
        use aegon_crypto::ecvrf::{Output, Proof};
...
impl EcVrfHash {
    ...
    pub fn public_key() -> aegon_crypto::ecvrf::VRFPublicKey {
        aegon_crypto::ecvrf::VRFPublicKey::from(vrf_secret_key())
    }
