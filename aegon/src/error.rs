...
pub enum AegonError {
    ...
    /// or trim).
    #[error("PCS error: {0:?}")]
    Pcs(aegon_crypto::pcs::prelude::PCSError),

    /// Propagated from the Fiat-Shamir transcript.
    #[error("transcript error: {0:?}")]
    Transcript(aegon_crypto::transcript::TranscriptError),
...
impl From<aegon_crypto::transcript::TranscriptError> for AegonError {
    fn from(e: aegon_crypto::transcript::TranscriptError) -> Self {
        AegonError::Transcript(e)
    }
}

impl From<aegon_crypto::pcs::prelude::PCSError> for AegonError {
    fn from(e: aegon_crypto::pcs::prelude::PCSError) -> Self {
        AegonError::Pcs(e)
    }
}
