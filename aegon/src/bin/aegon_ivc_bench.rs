...
//! [`aegon::ivc::synthetic`]). That is sound here because the
...
fn main() {
    ...
    for &n in &shard_counts {
        ...
        let vctx = VerifierContext::<Bn254, Pcs>::new(args.num_vars, vk.clone())
            .with_audit_fs(aegon::ivc::adapter::poseidon_audit_fs());
...
fn size_sweep(
    ...
    _vk: &aegon_crypto::pcs::kzhk::srs::KZHKVerifierParam<Bn254>,
) {
    let n: usize = args
        .shards
        .split(',')
...
fn snark_compare(args: &Args, h: G1Affine) {
    ...
    for &n in &shard_counts {
        ...
        assert_eq!(
            z[0],
            <aegon::ivc::prover::E1 as nova_snark::traits::Engine>::Scalar::from(steps as u64),
...
fn fs_compare(args: &Args) {
    use aegon::audit_fs::AuditFsHooks;
    use aegon_crypto::pcs::kzhk::structs::KZHKCommitment;
    ...
    let pos = aegon::ivc::adapter::poseidon_audit_fs();
