...
use aegon::distributed_srs::{
    run_distributed_compute, try_cache_hit, Phase, SrsBootstrapConfig, SrsBootstrapState,
    SrsServer as SrsGrpcServer,
};
use aegon::hash::VrfProver;
use aegon::server::load_aegon_checkpoint_from_db;
use aegon::shard_grpc::{ShardServer, ShardServerTlsConfig};
use aegon::sharded::read_srs_from_file;
use aegon::{AegonConfig, DbSource, EcVrfHash};
...
type Pcs = KZHK<Bn254>;
type Aegon = aegon::Aegon<Bn254, Pcs, EcVrfHash>;
...
struct Args {
    ...
    #[arg(long, default_value_t = aegon::audit_fs::AuditFs::Poseidon)]
    audit_fs: aegon::audit_fs::AuditFs,
}

#[tokio::main]
async fn main() -> ExitCode {
    #[cfg(feature = "tracing_instrument")]
    aegon::tracing_init::init_tree_subscriber();
    let args = Args::parse();

    aegon::instrument::set_rss_log(args.log_rss);
    aegon::instrument::log_rss("startup");
    ...
    aegon::instrument::log_rss("post_aegon_init");
    ...
    match args.masking_addr.len() {
        ...
        1 => {
            ...
            match aegon::masking::MaskingClient::<Bn254, Pcs>::connect(addr.clone()) {
        ...
        n => {
            ...
            match aegon::masking::MaskingClientPool::<Bn254, Pcs>::connect_all(&args.masking_addr) {
...
fn build_aegon_from_srs(
    pk: aegon_crypto::pcs::kzhk::srs::KZHKProverParam<Bn254>,
    vk: aegon_crypto::pcs::kzhk::srs::KZHKVerifierParam<Bn254>,
