...
use aegon::sharded::ShardedValueHistory;
use aegon::{
...
/// goal isn't to use _correct_ RSA bytes — the directory only sees the byte
...
fn publish_with_retry(
    ...
    context: &str,
) -> Result<tonic::Response<aegon::coordinator_grpc::proto::PublishResponse>, tonic::Status> {
...
fn main() -> ExitCode {
    ...
    let (state_opt, setup_ms, initial_prefill_ms) = if remote_coord_mode {
        ...
        let mut builder = ShardedAegonConfig::<Bn254, Pcs>::builder()
            ...
            .log_capacity(
                aegon::true_log_capacity_from_shard(args.shard_log_capacity) + log_n_shards,
            )
        ...
        let mut state = match Sharded::setup(&cfg) {
            Ok(s) => s,
            Err(e) => {
    ...
    for (level_idx, &target) in preload_counts.iter().enumerate() {
        ...
        while current_count < target {
            ...
            //     is what we time.
            let res: Result<(), aegon::AegonError> = if let Some(s) = &shared {
                ...
                match publish_with_retry(&raw_client, req, &driver_rt, &ctx) {
                    Ok(_resp) => Ok(()),
                    Err(e) => Err(aegon::AegonError::Config(format!("remote publish: {e}"))),
                }
            };
