...
//! ([`crate::ivc`]) has to *recompute* these challenges in
...
pub enum AuditFs {
    ...
    /// [`crate::ivc::adapter::poseidon_audit_fs`].
    #[default]
    Poseidon,
}
...
pub type ChainScalarFn<E, P> = fn(
    ...
    &[<P as aegon_crypto::pcs::PolynomialCommitmentScheme<E>>::Commitment],
) -> <E as Pairing>::ScalarField;
...
impl<E: Pairing, P: AegonPcs<E>> AuditFsHooks<E, P> {
    ...
    /// [`crate::ivc::adapter`] to install the Poseidon
