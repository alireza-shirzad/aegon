...
//! when set, otherwise defaults to `aegon=debug,aegon_crypto::pcs::kzhk=info` so every
...
///
/// Default filter is `aegon=debug,aegon_crypto::pcs::kzhk=info`
...
pub fn init_tree_subscriber() {
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("aegon=debug,aegon_crypto::pcs::kzhk=info"));
