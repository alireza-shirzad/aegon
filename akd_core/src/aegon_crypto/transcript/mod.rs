...
/// Compress-serialize `value` into a fresh `Vec<u8>` and return it as a
/// `Result` so callers can `?`-bubble serialization failures. Plays the
/// same role the `ark_std::to_bytes!` macro did in older arkworks
/// versions.
fn to_bytes<T: CanonicalSerialize>(value: &T) -> Result<Vec<u8>, TranscriptError> {
    let mut buf = Vec::with_capacity(value.compressed_size());
    value
        .serialize_compressed(&mut buf)
        .map_err(|e| TranscriptError::InvalidTranscript(format!("serialize_compressed: {e}")))?;
    Ok(buf)
}
